...
export default function App() {
  const [checked, setChecked] = useState(() => {
    const saved = localStorage.getItem("easyTrackerChecked");
    return saved ? JSON.parse(saved) : days.map(() => Array(tasks.length).fill(false));
  });

  useEffect(() => {
    localStorage.setItem("easyTrackerChecked", JSON.stringify(checked));
  }, [checked]);
  ...
  return (
    <div className="p-4 font-sans max-w-screen overflow-x-auto">
      <h1 className="text-3xl font-bold mb-2 text-center">Easy Tracker – 75 Day Challenge</h1>
      <p className="text-md italic text-gray-600 mb-6 text-center">
        Simple habits for health and energy.
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {tasks.map((task, i) => (
          <div key={i} className="flex flex-col items-center text-center p-3 border rounded-lg bg-white shadow-sm">
            <div className="text-4xl mb-2">{task.icon}</div>
            <div className="font-semibold text-sm mb-1">{task.label}</div>
